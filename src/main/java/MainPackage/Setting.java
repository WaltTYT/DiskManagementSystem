...
public class Setting {//设置类
    ...
    public static void handleInsertImage() {//处理插入图片
        JLabel backgroundImageDirectoryLabel = new JLabel(SettingState.systemLanguage ? "Picture Directory: " : "图片路径：");//背景图片路径标签
        backgroundImageDirectoryLabel.setPreferredSize(new Dimension(SettingState.systemLanguage ? 170 : 100, 34));//设置大小
        backgroundImageDirectoryLabel.setFont(new Font("微软雅黑", PLAIN, 20));//设置字体
        backgroundImageDirectoryLabel.setForeground(SettingState.themeColor ? DARK_DIALOG_FONT_COLOR : LIGHT_DIALOG_FONT_COLOR);//字体颜色
        backgroundImageDirectoryLabel.addMouseListener(new MouseAdapter() {//为背景图片路径标签添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Select Your Favorite Picture From Disk As The Software Background Image" : "从磁盘选择您喜欢的图片作为软件背景图片", backgroundImageDirectoryLabel));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
                backgroundImageDirectoryLabel.setForeground(Color.RED);//悬浮颜色
            }
            ...
            public void mouseExited(MouseEvent e) {//如果鼠标离开
                ...
                backgroundImageDirectoryLabel.setForeground(SettingState.themeColor ? DARK_DIALOG_FONT_COLOR : LIGHT_DIALOG_FONT_COLOR);//恢复颜色
            }
        });

        CustomTextField backgroundImageDirectoryTextField = new CustomTextField(400, new ImageIcon(new ImageIcon("src/material/image/clearText.png").getImage().getScaledInstance(24, 24, Image.SCALE_DEFAULT)), SettingState.systemLanguage ? "Background Picture Directory" : "背景图片路径");//背景图片路径文本域
        backgroundImageDirectoryTextField.getInputTextField().setPreferredSize(new Dimension(400, 34));//设置大小
        backgroundImageDirectoryTextField.getInputTextField().setFont(new Font("微软雅黑", PLAIN, 20));//设置字体
        backgroundImageDirectoryTextField.getInputTextField().setBackground(Color.white);//设置背景颜色
        backgroundImageDirectoryTextField.getActionButton().setBackground(SETTING_BUTTON_COLOR);//设置背景颜色
        backgroundImageDirectoryTextField.getActionButton().setBorder(null);//无边框
        if (!SettingState.backgroundPictureDirectory.isEmpty()) {//如果有背景图片
            backgroundImageDirectoryTextField.getInputTextField().setText(SettingState.backgroundPictureDirectory);//设置文本
        }
        backgroundImageDirectoryTextField.getInputTextField().addKeyListener(new KeyAdapter() {//为背景图片路径文本域添加键盘监听
            @Override
            public void keyPressed(KeyEvent e) {//如果键盘按下
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {//如果按下回车
                    String backgroundImageDirectoryInput = backgroundImageDirectoryTextField.getInputTextField().getText();//获取输入
                    if (backgroundImageDirectoryInput.isEmpty()) {//如果输入为空
                        SettingState.backgroundPictureDirectory = "";//清空
                        try {
                            disposeTransparentBackground();//释放背景图片
                        } catch (IOException ex) {
                            handleErrorLog(ex.getMessage());//处理错误日志
                            throw new RuntimeException(ex);//捕获异常
                        }
                    } else {//否则
                        if (backgroundImageDirectoryInput.charAt(0) == '\"' && backgroundImageDirectoryInput.charAt(backgroundImageDirectoryInput.length() - 1) == '\"') {//如果是复制的带双引号的字符串
                            backgroundImageDirectoryInput = backgroundImageDirectoryInput.substring(1, backgroundImageDirectoryInput.length() - 1);//进行截断
                            backgroundImageDirectoryTextField.getInputTextField().setText(backgroundImageDirectoryInput);//重新设置
                        }
                        File backgroundImageFile = new File(backgroundImageDirectoryInput);
                        if (!backgroundImageFile.exists()) {//如果图片存在且是图片文件
                            backgroundImageDirectoryTextField.getInputTextField().setText(SettingState.backgroundPictureDirectory);//返回值
                            createBottomTipWindow(SettingState.systemLanguage ? "Nonexistent Picture File Directory" : "该图片文件路径不存在");//提示
                        } else if (!detectPictureFileByMagicNumber(backgroundImageFile)) {
                            backgroundImageDirectoryTextField.getInputTextField().setText(SettingState.backgroundPictureDirectory);//返回值
                            createBottomTipWindow(SettingState.systemLanguage ? "Please Enter Picture File Directory" : "请输入图片文件路径");//提示
                        } else {//否则
                            if (SettingState.backgroundPictureDirectory.isEmpty()) {//如果背景图片路径为空
                                SettingState.backgroundPictureDirectory = backgroundImageDirectoryInput;//记录
                                initTransparentBackground();//初始化透明背景图片
                                createBottomTipWindow(SettingState.systemLanguage ? "Success To Set Background Picture" : "背景图片设置成功");//提示
                            }
                        }
                    }
                    e.consume();//阻止默认行为
                } else if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {//如果按下ESC
                    insertImageDialog.requestFocusInWindow();//焦点返回菜单
                    e.consume();//阻止默认行为
                }
            }
        });
        backgroundImageDirectoryTextField.getInputTextField().addFocusListener(new FocusAdapter() {//为背景图片路径文本域添加聚焦监听
            @Override
        ...
        backgroundImageDirectoryTextField.getInputTextField().addMouseListener(new MouseAdapter() {//为背景图片路径文本域添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Enter Directory Of Your Favorite Picture (You Can Directly Paste Picture Path Here)" : "输入您喜欢的图片路径（您可以直接粘贴图片路径到这里）", backgroundImageDirectoryTextField.getInputTextField()));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        backgroundImageDirectoryTextField.getActionButton().addActionListener(_ -> {//为清空按钮添加事件监听
            if (!SettingState.backgroundPictureDirectory.isEmpty()) {//如果有背景图片路径
                try {
                    disposeTransparentBackground();//释放背景图片
                } catch (IOException e) {
                    handleErrorLog(e.getMessage());//处理错误日志
            ...
            SettingState.backgroundPictureDirectory = "";//清空
            backgroundImageDirectoryTextField.getInputTextField().setText("");//设置文本
        });
        backgroundImageDirectoryTextField.getActionButton().addMouseListener(new MouseAdapter() {//为清空按钮添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Clear Background Image" : "清空背景图片", backgroundImageDirectoryTextField.getActionButton()));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        JButton pictureDirectorySelectButton = new JButton(SettingState.systemLanguage ? "Select A Picture" : "选择图片");//背景图片选择按钮
        pictureDirectorySelectButton.setPreferredSize(new Dimension(SettingState.systemLanguage ? 150 : 100, 34));//设置大小
        pictureDirectorySelectButton.setFont(new Font("微软雅黑", PLAIN, 20));//设置字体
        pictureDirectorySelectButton.setBackground(SETTING_BUTTON_COLOR);//设置背景颜色
        pictureDirectorySelectButton.setFocusable(false);//不可聚焦
        pictureDirectorySelectButton.setBorder(null);//无边框
        JFileChooser backgroundImageChooser = new JFileChooser();//背景图片选择器
        backgroundImageChooser.setFileSelectionMode(JFileChooser.FILES_ONLY);//设置选择器只能选中文件
        if (SettingState.pictureDirectory.isEmpty()) {//如果没有图片路径
            backgroundImageChooser.setCurrentDirectory(new File(System.getProperty("user.home")));//从用户目录寻找
        } else {//否则
            backgroundImageChooser.setCurrentDirectory(new File(SettingState.pictureDirectory));//从图片目录寻找
        }
        pictureDirectorySelectButton.addActionListener(_ -> {//为选择图片路径按钮添加事件监听
            int imageSelectState = backgroundImageChooser.showDialog(insertImageDialog, SettingState.systemLanguage ? "Select Background Picture Directory" : "选择背景图片路径");//打开文件选择器
            if (imageSelectState == JFileChooser.APPROVE_OPTION) {//如果确认选择
                File backgroundPictureFile = backgroundImageChooser.getSelectedFile();//获取选中文件
                if (detectPictureFileByMagicNumber(backgroundPictureFile)) {//如果是图片文件
                    SettingState.backgroundPictureDirectory = backgroundPictureFile.getAbsolutePath();//记录
                    backgroundImageDirectoryTextField.getInputTextField().setText(SettingState.backgroundPictureDirectory);//设置文本
                    initTransparentBackground();//初始化透明背景图片
                } else {//否则
                    createBottomTipWindow(SettingState.systemLanguage ? "Please Select Picture File" : "请选择图片文件");//提示
                }
            }
        });
        pictureDirectorySelectButton.addMouseListener(new MouseAdapter() {//为背景图片选择按钮添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Click To Select Your Favorite Picture From Disk As The Software Background Image" : "点击按钮从磁盘选择您喜欢的图片作为软件背景图片", pictureDirectorySelectButton));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        JPanel insertImageTopPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 5));//插入图片顶部面板
        insertImageTopPanel.setBackground(SettingState.themeColor ? DARK_DIALOG_MAIN_COLOR : LIGHT_DIALOG_MAIN_COLOR);//设置背景颜色
        insertImageTopPanel.add(backgroundImageDirectoryLabel);
        insertImageTopPanel.add(backgroundImageDirectoryTextField);
        insertImageTopPanel.add(pictureDirectorySelectButton);

        JLabel backgroundImageOpacityLabel = new JLabel(SettingState.systemLanguage ? "Picture Opacity: " : "不透明度：");//背景图片不透明度标签
        backgroundImageOpacityLabel.setPreferredSize(new Dimension(SettingState.systemLanguage ? 170 : 100, 34));//设置大小
        backgroundImageOpacityLabel.setFont(new Font("微软雅黑", PLAIN, 20));//设置字体
        backgroundImageOpacityLabel.setForeground(SettingState.themeColor ? DARK_DIALOG_FONT_COLOR : LIGHT_DIALOG_FONT_COLOR);//字体颜色
        backgroundImageOpacityLabel.addMouseListener(new MouseAdapter() {//为背景图片不透明度标签添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Adjust Opacity Of Background Image" : "调整背景图片不透明度", backgroundImageOpacityLabel));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
                backgroundImageOpacityLabel.setForeground(Color.RED);//悬浮颜色
            }
            ...
            public void mouseExited(MouseEvent e) {//如果鼠标离开
                ...
                backgroundImageOpacityLabel.setForeground(SettingState.themeColor ? DARK_DIALOG_FONT_COLOR : LIGHT_DIALOG_FONT_COLOR);//恢复颜色
            }
        });

        JSlider backgroundImageOpacitySlider = new JSlider(JSlider.HORIZONTAL, 0, 100, 50);//背景图片不透明度拖动条
        backgroundImageOpacitySlider.setPreferredSize(new Dimension(400, 34));//设置大小
        backgroundImageOpacitySlider.setValue(SettingState.imageOpacity);//设置值
        backgroundImageOpacitySlider.setBackground(SettingState.themeColor ? DARK_DIALOG_MAIN_COLOR : LIGHT_DIALOG_MAIN_COLOR);//设置背景颜色
        backgroundImageOpacitySlider.addMouseListener(new MouseAdapter() {//为背景图片不透明度拖动条添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Drag To Adjust Opacity Of Background Image" : "拖动调整背景图片不透明度", backgroundImageOpacitySlider));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        JTextField backgroundImageOpacityTextField = new JTextField();//背景图片不透明度文本域
        backgroundImageOpacityTextField.setPreferredSize(new Dimension(SettingState.systemLanguage ? 150 : 100, 34));//设置大小
        backgroundImageOpacityTextField.setFont(new Font("微软雅黑", PLAIN, 20));//设置字体
        backgroundImageOpacityTextField.setText(String.valueOf(SettingState.imageOpacity));//设置值
        backgroundImageOpacityTextField.setBackground(Color.white);//设置背景颜色
        backgroundImageOpacitySlider.addChangeListener(_ -> {//为不透明度拖动条添加变化监听
            if (backgroundImageOpacitySlider.hasFocus()) {//如果有焦点
                SettingState.imageOpacity = backgroundImageOpacitySlider.getValue();//记录
                backgroundImageOpacityTextField.setText(String.valueOf(backgroundImageOpacitySlider.getValue()));//设置文本
                if (!SettingState.backgroundPictureDirectory.isEmpty()) {//如果有图片
                    try {
                        updateTransparentBackground();//设置透明背景图片
                    } catch (IOException e) {
                        handleErrorLog(e.getMessage());//处理错误日志
                        throw new RuntimeException(e);//捕获异常
                    }
        ...
        backgroundImageOpacityTextField.addKeyListener(new KeyAdapter() {//为背景图片不透明度文本域添加键盘监听
            @Override
            public void keyPressed(KeyEvent e) {//如果键盘按下
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {//如果按下回车
                    String backgroundImageOpacityInput = backgroundImageOpacityTextField.getText();//获取输入
                    if (backgroundImageOpacityInput.isEmpty()) {//如果输入为空
                        backgroundImageOpacityTextField.setText(String.valueOf(SettingState.imageOpacity));//返回值
                        createBottomTipWindow(SettingState.systemLanguage ? "Input Cannot Be Empty" : "输入不可为空");//提示
                    } else {//否则
                        for (int i = 0; i < backgroundImageOpacityInput.length(); i++) {//遍历输入文本
                            if (!Character.isDigit(backgroundImageOpacityInput.charAt(i))) {//如果非数字字符
                                backgroundImageOpacityTextField.setText(String.valueOf(SettingState.imageOpacity));//返回值
                                createBottomTipWindow(SettingState.systemLanguage ? "No Non Digit Character" : "请勿输入非数字字符");//提示
                                return;//直接返回
                            }
                        }
                        int backgroundImageOpacityValue = Integer.parseInt(backgroundImageOpacityInput);//获取值
                        if (backgroundImageOpacityValue < 0 || backgroundImageOpacityValue > 100) {//如果越界
                            backgroundImageOpacityTextField.setText(String.valueOf(SettingState.imageOpacity));//返回值
                            createBottomTipWindow(SettingState.systemLanguage ? "Please Do Not Enter Number Less Than 0 Or Greater Than 100" : "请勿输入小于0或大于100的数字");//提示
                        } else {//否则
                            SettingState.imageOpacity = backgroundImageOpacityValue;//记录
                            backgroundImageOpacitySlider.setValue(backgroundImageOpacityValue);//设置值
                            if (!SettingState.backgroundPictureDirectory.isEmpty()) {//如果有图片
                                try {
                                    updateTransparentBackground();//设置透明背景图片
                                } catch (IOException ex) {
        ...
        backgroundImageOpacityTextField.addFocusListener(new FocusAdapter() {//为背景图片不透明度文本域添加聚焦监听
            @Override
            public void focusLost(FocusEvent e) {//如果失去聚焦
                String backgroundImageOpacityInput = backgroundImageOpacityTextField.getText();//获取输入
                if (backgroundImageOpacityInput.isEmpty()) {//如果输入为空
        ...
        backgroundImageOpacityTextField.addMouseListener(new MouseAdapter() {//为背景图片不透明度文本域添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow(SettingState.systemLanguage ? "Enter To Adjust Opacity Of Background Image" : "输入调整背景图片不透明度", backgroundImageOpacityTextField));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        JPanel insertImageBottomPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 5));//插入图片底部面板
        insertImageBottomPanel.setBackground(SettingState.themeColor ? DARK_DIALOG_MAIN_COLOR : LIGHT_DIALOG_MAIN_COLOR);//设置背景颜色
        insertImageBottomPanel.add(backgroundImageOpacityLabel);
        insertImageBottomPanel.add(backgroundImageOpacitySlider);
        insertImageBottomPanel.add(backgroundImageOpacityTextField);

        insertImageDialog = new JDialog(pictureManagementSystemFrame, SettingState.systemLanguage ? "Insert Image" : "插入图片", true);//创建插入图片对话窗口
        insertImageDialog.setIconImage(new ImageIcon("src/material/image/backgroundImage.png").getImage());//设置图标
        insertImageDialog.setLayout(new BorderLayout());//设置布局
        insertImageDialog.add(insertImageTopPanel, BorderLayout.NORTH);
        insertImageDialog.add(insertImageBottomPanel, BorderLayout.CENTER);
        insertImageDialog.pack();//合适
        insertImageDialog.setLocation(screenSize.width / 2 - insertImageDialog.getWidth() / 2, screenSize.height / 2 - insertImageDialog.getHeight() / 2);//设置位置
        insertImageDialog.setVisible(true);//最后设置可见
        JRootPane insertImageRoot = insertImageDialog.getRootPane();//获取插入图片窗口的根
        insertImageRoot.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "closeInsertImageDialog");//为根设置窗口关闭ESC按键绑定
        insertImageRoot.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_U, KeyEvent.CTRL_DOWN_MASK), "closeInsertImageDialog");//为根设置窗口关闭Ctrl+U按键绑定
        insertImageRoot.getActionMap().put("closeInsertImageDialog", new AbstractAction() {//当ESC按键执行时
            public void actionPerformed(ActionEvent event) {//行为执行
                insertImageDialog.dispatchEvent(new WindowEvent(insertImageDialog, WindowEvent.WINDOW_CLOSING));//关闭窗口
            }
        });
        insertImageDialog.addWindowListener(new WindowAdapter() {//为插入图片窗口添加窗口监听
            @Override
    ...
    public static void initTransparentBackground() {//初始化透明背景
        try {
            if (SettingState.backgroundPictureDirectory.startsWith("My Cloud\\")) {//如果是云盘背景图片
                backgroundImagePanel = new BackgroundImagePanel(ImageIO.read(Objects.requireNonNull(User.handleUserLoadUserUploadPicture(Collections.singletonList(SettingState.backgroundPictureDirectory.substring(9))))[0]), SettingState.imageOpacity / 100f);//设置背景图片
            } else {//否则
                backgroundImagePanel = new BackgroundImagePanel(ImageIO.read(new File(SettingState.backgroundPictureDirectory)), SettingState.imageOpacity / 100f);//设置背景图片
            }
            DirectoryTree.directoryTree.setBackground(new Color(0, 0, 0, 0));//设置无背景
            DirectoryTree.directoryTree.setOpaque(false);//设置透明
            DirectoryTree.directoryTree.setCellRenderer(new DriveTreeRenderer());//设置自定义节点渲染器
            FileDisplayBottomBar.zoomSlider.setBackground(null);//设置无背景
            FileDisplayBottomBar.zoomSlider.setOpaque(false);//设置透明
            FileDisplayBottomBar.bottomBarPanel.setBackground(null);//设置无背景
            FileDisplayBottomBar.bottomBarPanel.setOpaque(false);//设置透明
            mainPanel.setBackground(null);//设置无背景
            mainPanel.setOpaque(false);//设置透明
            topBarPanel.setBackground(null);//设置无背景
            topBarPanel.setOpaque(false);//设置透明
            directoryTreeScrollPane.setBackground(null);//设置无背景
            directoryTreeScrollPane.setOpaque(false);//设置透明
            directoryTreeScrollPane.getViewport().setBackground(null);//设置无背景
            directoryTreeScrollPane.getViewport().setOpaque(false);//设置透明
            picturePreviewBottomBarPanel.setBackground(null);//设置无背景
            picturePreviewBottomBarPanel.setOpaque(false);//设置透明
            picturePreviewMainPanelScrollPane.setBackground(null);//设置无背景
            picturePreviewMainPanelScrollPane.setOpaque(false);//设置透明
            picturePreviewMainPanelScrollPane.getViewport().setBackground(null);//设置无背景
            picturePreviewMainPanelScrollPane.getViewport().setOpaque(false);//设置透明
            picturePreviewTopBarPanel.setBackground(null);//设置无背景
            picturePreviewTopBarPanel.setOpaque(false);//设置透明
            picturePreviewPanel.setBackground(null);//设置无背景
            picturePreviewPanel.setOpaque(false);//设置透明
            panelSplitPane.setBackground(null);//设置无背景
            panelSplitPane.setOpaque(false);//设置透明
            bottomTipInformationPanel.setBackground(null);//设置无背景
            bottomTipInformationPanel.setOpaque(false);//设置透明
            bottomTipInformation.setBackground(null);//设置无背景
            bottomTipInformation.setOpaque(false);//设置透明

            backgroundImagePanel.revalidate();//重新验证布局
            backgroundImagePanel.repaint();//重新绘制
            pictureManagementSystemFrame.revalidate();//重新验证布局
            pictureManagementSystemFrame.repaint();//重新绘制
            refreshScrollBars();//刷新滚动栏
        } catch (IOException e) {
            SettingState.backgroundPictureDirectory = "";//清空
            try {
                disposeTransparentBackground();//释放背景图片
            ...
            handleErrorLog(e.getMessage());//处理错误日志
            createBottomTipWindow(SettingState.systemLanguage ? "Unable To Get Your Background Image, Please Reset" : "无法获取您的背景图片，请重新设置");//提示
        }
    }

    public static void updateTransparentBackground() throws IOException {//更新透明背景
        backgroundImagePanel.setAlpha(SettingState.imageOpacity / 100f);//设置不透明度
        backgroundImagePanel.revalidate();//重新验证布局
        backgroundImagePanel.repaint();//重新绘制
        pictureManagementSystemFrame.revalidate();//重新验证布局
        pictureManagementSystemFrame.repaint();//重新绘制
    ...
    public static void disposeTransparentBackground() throws IOException {//释放透明背景
        DirectoryTree.directoryTree.setBackground(SettingState.themeColor ? DARK_DIRECTORY_MAIN_COLOR : LIGHT_DIRECTORY_MAIN_COLOR);//设置背景
        DirectoryTree.directoryTree.setOpaque(true);//设置不透明
        DirectoryTree.directoryTree.setCellRenderer(new DriveTreeRenderer());//设置自定义节点渲染器
        FileDisplayBottomBar.zoomSlider.setBackground(SettingState.themeColor ? DARK_PICTURE_BAR_COLOR : LIGHT_PICTURE_BAR_COLOR);//设置背景
        FileDisplayBottomBar.zoomSlider.setOpaque(true);//设置不透明
        FileDisplayBottomBar.bottomBarPanel.setBackground(SettingState.themeColor ? DARK_PICTURE_BAR_COLOR : LIGHT_PICTURE_BAR_COLOR);//设置背景
        FileDisplayBottomBar.bottomBarPanel.setOpaque(true);//设置不透明
        mainPanel.setBackground(SettingState.themeColor ? DARK_PICTURE_MAIN_COLOR : LIGHT_PICTURE_MAIN_COLOR);//设置背景
        mainPanel.setOpaque(true);//设置不透明
        topBarPanel.setBackground(SettingState.themeColor ? DARK_PICTURE_BAR_COLOR : LIGHT_PICTURE_BAR_COLOR);//设置背景
        topBarPanel.setOpaque(true);//设置不透明
        ...
        bottomTipInformationPanel.setBackground(SettingState.themeColor ? DARK_TIP_IMFORMATION_COLOR : LIGHT_TIP_IMFORMATION_COLOR);//设置背景
        bottomTipInformationPanel.setOpaque(true);//设置不透明
        bottomTipInformation.setBackground(SettingState.themeColor ? DARK_TIP_IMFORMATION_COLOR : LIGHT_TIP_IMFORMATION_COLOR);//设置背景
        bottomTipInformation.setOpaque(true);//设置不透明
    ...
    public static void refreshScrollBars() {//刷新滚动栏
        refreshScrollBar(directoryTreeScrollPane.getVerticalScrollBar());//重绘滚动栏
        refreshScrollBar(directoryTreeScrollPane.getHorizontalScrollBar());//重绘滚动栏
        refreshScrollBar(picturePreviewMainPanelScrollPane.getVerticalScrollBar());//重绘滚动栏
        refreshScrollBar(picturePreviewMainPanelScrollPane.getHorizontalScrollBar());//重绘滚动栏
    }

    private static void refreshScrollBar(JScrollBar scrollBar) {//重绘滚动栏
        if (scrollBar.getUI() instanceof CustomScrollPane) {//如果属于自定义滚动栏实例
    ...
    public static void switchSystemLanguage() {//切换系统语言
        ...
        FileDisplayPopupMenu.setPictureMenu.setText(systemLanguage ? "Set Picture As" : "将图片设置为");//清空图片回收站按钮（任何时候有效）
        FileDisplayPopupMenu.setPictureAsBackgroundPictureButton.setText(systemLanguage ? "Background Picture (Ctrl + B)" : "背景图片（Ctrl + B）");//清空图片回收站按钮（任何时候有效）
        FileDisplayPopupMenu.setPictureAsLockscreenButton.setText(systemLanguage ? "Lock Screen (Ctrl + Q)" : "锁屏（Ctrl + Q）");//清空图片回收站按钮（任何时候有效）
        FileDisplayPopupMenu.setPictureAsWallpaperButton.setText(systemLanguage ? "Wall Paper (Ctrl + W)" : "壁纸（Ctrl + W）");//清空图片回收站按钮（任何时候有效）
    ...
    public static void initSettingDialog() {//初始化设置菜单
        ...
        JButton pictureDirectoryButton = new JButton(SettingState.systemLanguage ? "Select Picture Directory" : "选择图片路径");//选择图片路径按钮
        pictureDirectoryButton.setBackground(SETTING_BUTTON_COLOR);//设置背景颜色
        pictureDirectoryButton.setFont(new Font("楷体", PLAIN, 20));//设置字体
        JButton restoreDefaultButton = new JButton(SettingState.systemLanguage ? "Restore To Default" : "恢复默认路径");//恢复默认路径按钮
        restoreDefaultButton.setBackground(SETTING_BUTTON_COLOR);//设置背景颜色
        restoreDefaultButton.setFont(new Font("楷体", PLAIN, 20));//设置字体
        JFileChooser pictureDirectoryChooser = new JFileChooser();//图片路径选择器
        pictureDirectoryChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);//设置选择器只能选中文件夹
        pictureDirectoryChooser.setCurrentDirectory(new File(System.getProperty("user.home")));//默认从用户目录寻找
        pictureDirectoryButton.addActionListener(_ -> {//为选择图片路径按钮添加事件监听
            int directorySelectState = pictureDirectoryChooser.showDialog(settingDialog, SettingState.systemLanguage ? "Select System Picture Directory" : "选择系统图片路径");//打开文件选择器
            if (directorySelectState == JFileChooser.APPROVE_OPTION) {//如果确认选择
                if (!SettingState.pictureDirectory.isEmpty()) {//如果图片路径不为空
                    rootNode.remove(pictureNode);//移除原先图片结点
                }
                SettingState.pictureDirectory = pictureDirectoryChooser.getSelectedFile().getAbsolutePath();//更新图片路径
                pictureNode = new DefaultMutableTreeNode(new File(SettingState.pictureDirectory));//创建图片结点
                pictureNode.add(new DefaultMutableTreeNode(new Placeholder()));//为其添加占位符
                if (!SettingState.userAccount.isEmpty()) {//如果用户登录
                    rootNode.remove(cloudNode);//暂时移除云盘结点
                    rootNode.add(pictureNode);//将图片结点添加到根结点中
                    rootNode.add(cloudNode);//再添加云盘结点
                } else {//否则
                    rootNode.add(pictureNode);//将图片结点添加到根结点中
                }
                DirectoryTree.directoryTree.setModel(new DefaultTreeModel(rootNode));//重新设置目录树模型
        ...
        pictureDirectoryButton.addMouseListener(new MouseAdapter() {//为选择图片路径按钮添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow((SettingState.systemLanguage ? "Manually Set Your System Picture Path" : "手动设置您的系统图片路径"), pictureDirectoryButton));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        restoreDefaultButton.addActionListener(_ -> {//为恢复默认路径按钮添加事件监听
            int confirm = JOptionPane.showConfirmDialog(settingDialog, (SettingState.systemLanguage ? "Are You Sure To Restore Default Picture Folder Directory?" : "确定要恢复默认图片文件夹路径吗？"), SettingState.systemLanguage ? "Restore To Default Confirm" : "恢复默认确认", JOptionPane.YES_NO_OPTION);//创建确认信息
            if (confirm == JOptionPane.YES_OPTION) {//如果确认
                ...
                if (picturesDir.exists() && picturesDir.isDirectory()) {//验证路径是否存在且路径是文件夹
                    ...
                    SettingState.pictureDirectory = picturesDir.getAbsolutePath();//将图片地址存档
                    pictureNode = new DefaultMutableTreeNode(new File(SettingState.pictureDirectory));//创建图片结点
                    pictureNode.add(new DefaultMutableTreeNode(new Placeholder()));//为其添加占位符
                    if (!SettingState.userAccount.isEmpty()) {//如果用户登录
                        rootNode.remove(cloudNode);//暂时移除云盘结点
                    ...
                    createBottomTipWindow(SettingState.systemLanguage ? "Unable To Found System Picture Folder Directory, Please Select Manually" : "未找到系统图片文件夹路径，请手动选择");//报错
                }
            }
        });
        restoreDefaultButton.addMouseListener(new MouseAdapter() {//为恢复默认路径按钮添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow((SettingState.systemLanguage ? "Reset Your System Picture Path To Default User Directory Path (Please Note That If Unable To Find Path, You Should Enter Manually)" : "重置您的系统图片路径为默认用户目录路径（请注意，如果无法找到路径请手动输入）"), restoreDefaultButton));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        contentPanel.add(pictureDirectoryButton);//面板添加选择图片路径按钮
        contentPanel.add(restoreDefaultButton);//面板添加恢复默认路径按钮
        ...
        JButton insertImageButton = new JButton(SettingState.systemLanguage ? "Background Image" : "背景图片");//插入图片按钮
        insertImageButton.setBackground(SETTING_BUTTON_COLOR);//设置背景颜色
        insertImageButton.setFont(new Font("楷体", PLAIN, 20));//设置字体
        insertImageButton.addActionListener(_ -> {//为插入图片按钮添加事件监听
            handleInsertImage();//处理插入图片
        });
        insertImageButton.addMouseListener(new MouseAdapter() {//为插入图片按钮添加鼠标事件监听
            @Override
            public void mouseEntered(MouseEvent e) {//如果鼠标进入
                hoverTimer = new Timer(1000, _ -> showButtonHoverTipWindow((SettingState.systemLanguage ? "Insert A Image You Like In Software Background" : "在软件背景插入您喜欢的图片"), insertImageButton));//展示提示窗口（鼠标悬浮一秒后展示）
                hoverTimer.setRepeats(false);//设置计时器不重复
                hoverTimer.start();//开始计时
            }
        ...
        contentPanel.add(insertImageButton);//面板添加插入图片按钮
        ...
        resetSettingButton.addActionListener(_ -> {//为插入图片按钮添加事件监听
            ...
            if (confirm == JOptionPane.YES_OPTION) {//如果确认
                ...
                try {
                    disposeTransparentBackground();//释放透明背景
                } catch (IOException e) {
                    handleErrorLog(e.getMessage());//处理错误日志
                    throw new RuntimeException(e);//捕获异常
                }
